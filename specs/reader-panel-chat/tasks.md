...
## Phase 11: 답변 하단 인용 출처 배지

- [x] [T017] assistant-ui `Sources`의 document source를 답변 완료 후 하단에 페이지별 인용 출처로 표시하고, 각 배지 아래에서 원문 미리보기와 PDF 위치 이동을 제공한다. 문장별 위치는 추측하지 않았다. 관련 테스트 4개 파일 62개와 포맷·린트·타입 검사, 프로덕션 빌드가 통과했다. 전체 테스트에서는 변경과 무관한 기존 테스트가 간헐적으로 실패했지만 해당 파일을 단독 실행하면 통과했다. (FR-014, SC-012)
