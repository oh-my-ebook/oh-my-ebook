...
- [ ] [T018] `src/features/reader/hooks/use-reader-layout.test.ts`, `src/features/reader/components/reader-toolbar.test.tsx`, `src/features/reader/components/reader.test.tsx`에 보기 전환·폭 제한·선호 복원·현재 페이지 유지 테스트를 먼저 작성해 실패를 확인한다. `src/features/reader/hooks/use-reader-layout.ts`에 화면 폭의 `matchMedia` 측정·정리를 추가하고, `src/features/reader/components/reader-toolbar.tsx`, `src/features/reader/components/reader.tsx`, `src/features/reader/components/page-navigator.tsx`에 보기 전환과 두 페이지 탐색을 연결한다. ToggleGroup 해제가 빈 배열이면 선택을 유지하고 제한 사유는 hover 없이도 확인할 수 있게 한다. 키보드 전환과 포커스를 보존한다. (FR-003, FR-005, FR-010, FR-011, FR-016, FR-019)
