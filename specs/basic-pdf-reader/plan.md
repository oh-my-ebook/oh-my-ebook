...
- 높이 맞춤 배율은 `min(가용 높이 / 표시 묶음의 최대 높이, 가용 너비 / 표시 묶음의 전체 폭)`으로 계산한다. 두 페이지 전체 폭에는 페이지 사이 간격을 포함하고 기존 디자인의 최대 1100px를 적용한다. 자동 맞춤에서는 표시 묶음 전체가 읽기 영역 안에 들어와 가로·세로 스크롤이 생기지 않아야 하며, 폭이 남으면 가로 중앙에 배치한다.
