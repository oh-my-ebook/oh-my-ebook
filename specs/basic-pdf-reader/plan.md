...
1. 페이지 방향·함께 표시할 페이지·탐색·배율 계산의 실패 테스트를 먼저 작성하고 순수 계산을 구현한다.
2. PDF.js와 샘플을 연결하고, 문서 로딩·페이지 렌더링·오류·취소의 실패 테스트를 거쳐 실제 PDF 한 장을 표시한다.
3. 기존 테마와 shadcn 컴포넌트를 조합해 툴바·페이지 탐색·보기 전환·크기 조절을 연결한다.
4. 읽기 영역 측정, 좁은 화면 전환, 빈 보조 패널, 포커스 복원을 구현한다.
5. 기존 시작 화면 테스트를 교체하고 실제 샘플을 사용하는 E2E와 브라우저 확대 검증을 완료한다.

각 단계는 후속 `tasks.md`에서 작은 작업으로 나누고, 작업마다 실패 확인 → 최소 구현 → 정리 순서로 진행한다. 한 작업을 완료하면 결과를 보고하고 다음 작업은 시작하지 않는다.
