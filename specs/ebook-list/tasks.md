...
- [x] [T001] `package.json`, `pnpm-lock.yaml`, `vite.config.ts`, `vercel.json`에
      `@sqlite.org/sqlite-wasm`, Vite 사전 번들 제외, dev·preview·Vercel COOP/COEP 헤더를 추가하고
      `pnpm build`와 응답 헤더 확인으로 설정을 검증한다.
- [x] [T002] T001 후 `pnpm dlx shadcn@latest add alert-dialog card empty progress --dry-run`과
      ...
      코드를 검증한다.
- [x] [T003] [P] `e2e/fixtures/ebook/`에 메타데이터가 있는 PDF, 없는 PDF, 회전된 1페이지 PDF,
      암호 PDF, 손상 파일과 동일 내용·다른 파일명 fixture를 추가하고 페이지 수·메타데이터·해시가
      의도와 같은지 검증한다. 테스트 실행 중 fixture를 생성하지 않는다.xs
...
- [x] [T005] `src/features/ebook-list/lib/ebook-store-client.test.ts`에 요청 ID 매칭, transferable
      ...
      해당 테스트를 통과시킨다.
- [x] [T006] [P] `src/features/ebook-list/lib/storage-manager.test.ts`와
      ...
      테스트를 통과시킨다.
- [x] [T007] T005 후 `src/features/ebook-list/components/ebook-library.test.tsx`에 초기 로딩 중 조작
      ...
      FR-016)
- [x] [T008] T005와 T007 후 `e2e/ebook-list.spec.ts`에 실제 `/` 진입 시 OPFS DB 초기화와 빈 책장
...
- [x] [T009] [P] [US1] `src/features/ebook-list/lib/pdf-import.test.ts`에 SHA-256 내용 식별,
...
- [x] [T010] T009 후 [US1] `src/features/ebook-list/lib/pdf-import.test.ts`에 첫 페이지 회전·비율,
      ...
      (FR-007, FR-008)
- [x] [T011] [P] [US1] `src/features/ebook-list/components/pdf-upload.test.tsx`에 접근 가능한 다중
...
- [x] [T012] [P] [US1] `src/features/ebook-list/components/storage-summary.test.tsx`에 사용량·예상
      ...
      (FR-031, FR-032)
- [x] [T013] T006, T009~T012 후 [US1] `src/features/ebook-list/components/ebook-library.test.tsx`에
...
- [x] [T014] T005, T008, T013 후 [US1] `e2e/ebook-list.spec.ts`에 유효한 PDF 업로드, 내용 중복,
...
- [x] [T015] T010, T014 후 [US1] `src/features/ebook-list/components/ebook-library.test.tsx`와
...
- [x] [T016] T014 후 [US1] `e2e/ebook-persistence.spec.ts`에 임시 user data 디렉터리의 persistent
...
- [x] [T017] [P] [US2] `src/features/ebook-list/components/book-card.test.tsx`에 표지·제목·지은이·
...
- [x] [T018] [P] [US2] 기본 Reader 완료 후 `src/features/reader/hooks/use-pdf-document.test.ts`와
      ...
      (FR-019~FR-022)
- [x] [T019] T005, T018 후 [US2] `src/pages/ebook-reader-page.test.tsx`에 책 로딩, 저장된 초기
      ...
      FR-037)
- [x] [T020] T014, T017, T019 후 [US2] `e2e/ebook-reader-progress.spec.ts`에 책 열기, 읽지 않음,
...
- [x] [T021] T006, T017 후 [US2] `src/features/ebook-list/components/ebook-library.test.tsx`에 수동
...
- [x] [T023] [P] [US3] T017 후 `src/features/ebook-list/components/ebook-shelf.test.tsx`에 여러 책의
      ...
      FR-027)
- [x] [T024] T017, T023 후 [US3] `src/features/ebook-list/components/book-card.test.tsx`에 포인터
      ...
      (FR-024, FR-025)
- [x] [T025] T023, T024 후 [US3] `e2e/ebook-list.spec.ts`에 정밀 포인터 3D 복귀, 키보드 전용
...
- [x] [T026] [P] [US4] `src/features/ebook-list/components/delete-book-dialog.test.tsx`에 책 제목과
...
- [x] [T027] T014, T017, T026 후 [US4] `src/features/ebook-list/components/ebook-library.test.tsx`와
