...
- 랜딩 최대 폭 1120px, 좌우 여백 16–32px. 기존 서재와 동일한 `background`·`card` 토큰과 56px 탐색 바를 사용한다.
- 랜딩 히어로는 32–48px Pretendard 제목과 직접 만든 SVG 표지의 입체 배치를 사용한다. 표지 전환은 240ms, 첫 등장만 700ms로 표현하고 동작 감소 설정에서는 제거한다.
