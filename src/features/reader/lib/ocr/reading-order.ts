...
function findGutter(lines: readonly OcrLine[], minWidth: number): Gutter | undefined {
  const events = lines
    .flatMap(({ bbox }) => [
      { x: bbox.x0, step: 1 },
      { x: bbox.x1, step: -1 },
...
function isRowAligned(left: readonly OcrLine[], right: readonly OcrLine[], lineHeight: number) {
  const [fewer, more] = left.length <= right.length ? [left, right] : [right, left]
  const centerY = ({ bbox }: OcrLine) => (bbox.y0 + bbox.y1) / 2
...
function isTextColumn(lines: readonly OcrLine[], lineHeight: number) {
...
function orderAroundGutter(lines: readonly OcrLine[], minGap: number): OcrLine[] | undefined {
  ...
  const crosses = ({ bbox }: OcrLine) => bbox.x0 < gutter.x1 && bbox.x1 > gutter.x0
